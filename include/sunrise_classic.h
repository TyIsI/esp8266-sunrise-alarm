#ifndef SUNRISE_OLD_H
#define SUNRISE_OLD_H

#include "platform.h"

#if SUNRISE_METHOD == SUNRISE_CLASSIC
...
int drawSunriseFrame()
{
  ...
  for (int row = 0; row < NUM_LEDS / LEDS_PER_ROW; row++)
  {
    ...
    if ((rowFrame < 0) || (rowFrame >= SUNRISE_FRAMES))
      continue;

    setPixel(p, sunrise[rowFrame][0], sunrise[rowFrame][1], sunrise[rowFrame][2]);
    more = 1;
  }
  ...
  if (frame > SUNRISE_FRAMES - WHITE_OFFSET)
  {
    ...
    fillWhite(level * PWM_RANGE_FULL);
  }
